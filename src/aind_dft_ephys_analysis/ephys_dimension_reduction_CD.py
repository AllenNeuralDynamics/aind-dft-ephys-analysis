...
def extract_trial_unit_rates(
    ...
    if align == "go_cue":
        ...
        var, trial_dim, trial_coord = "psth_reward_go_cue_start", "trial_reward_go_cue_start", "trial_index_reward_go_cue_start"
    else:
...
def extract_trial_unit_timecube(
    ...
    trial_ids = psth_da[trial_coord].values if trial_coord in psth_da.coords else np.arange(cube.shape[0])
...
def coding_direction_from_psth(
    ...
    *,
    ...
    fit_ext = extract_trial_unit_rates(psth_da, align=align, time_window=time_window, zscore_units=False)
    ...
    folds = [
        dict(train=np.concatenate([split.idx_train_a, split.idx_train_b]),
    ...
    if two_fold_cv:
        folds.append(
            dict(train=np.concatenate([split.idx_test_a, split.idx_test_b]),
    ...
    cube_ext = extract_trial_unit_timecube(psth_da, align=align, time_window=projection_time_window)
    ...
    labels_all_pm1 = np.concatenate([np.ones(len(idx_a_all), float), -np.ones(len(idx_b_all), float)])
    perm = np.argsort(np.concatenate([idx_a_all, idx_b_all]))  # reorder labels to match sorted indices
    ...
    out = {
        ...
        "metrics": {
            ...
            "overall": {
                "dprime": float((np.nanmean(y_test_norm[teA_mask]) - np.nanmean(y_test_norm[teB_mask])) /
                                (np.sqrt(0.5 * (np.nanvar(y_test_norm[teA_mask]) + np.nanvar(y_test_norm[teB_mask]))) + 1e-12)),
        ...
        "final_all": {"axis_w": w_final_all, "mu": mu_all, "sigma": sigma_all, "idx_all": idx_all_inc, "labels_all_pm1": labels_all_pm1},
    ...
    # 8) Optional save (includes TRAIN/TEST combined and per-class splits)
    if save_path is not None:
        ...
        if fmt == "npz":
            ...
            # xarray Dataset: store normalized arrays; split dims for clarity
            ds = xr.Dataset(
                data_vars={
                    # combined
